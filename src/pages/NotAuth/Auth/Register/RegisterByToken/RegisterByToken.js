...
import { checkNewEmail, registerByInviteLink } from "../../../../../store/actions/authActions";
...
function RegisterEasy({
    ...
    const register = () => {
        if (isCheckEmail) {
            dispatcher(registerByInviteLink(Object.assign(email, password, passwordConfirm, { hotelId: hotelId, token: token }, { agreeToTermOfUse: !!Number(agreeToTermOfUse) })))
        } else {
            dispatcher(checkNewEmail(email))
        }
    }
    ...
    return isCheckEmail ? formPassword : formEmail
}
